...
---
## ⚠️ 关于本地 Docker 部署(不建议 仅作Demo演示)

原因如下：

1. **检测结果不准确**: IP 检测是通过 Docker 内的 Clash 代理发起请求，检测的是**代理节点出口 IP**。如果你在本地运行，本地网络环境与云服务器不同，可能影响检测精度。

2. **本地网络干扰**: 本地可能存在防火墙、路由器 NAT、ISP 干扰等因素，导致部分节点无法正常连接或检测超时。

3. **资源占用**: Clash 需要切换大量节点并发起 HTTP 请求，会占用本地网络带宽和系统资源。
